...
// --- FETCHERS ---
const fetchCharacters = async (page: number, name: string): Promise<ApiResponse> => {
...
export default function CharactersPage() {
      ...
      toast.custom((t) => (
        <div className="bg-[#0a0a0a] border border-white/10 w-95 h-18 rounded-2xl shadow-2xl backdrop-blur-xl flex items-center px-6 gap-4 overflow-hidden relative">
          ...
          <div className="flex-1 text-[12px]">
            <span className="font-black text-white italic uppercase block mb-0.5">Acceso Denegado</span>
            Identificación requerida para calificar.
          </div>
          <button onClick={() => toast.dismiss(t)}><X size={18} className="text-slate-600" /></button>
        </div>
      ), { duration: 5000 });
      return;
    }
  ...
  const Pagination = () => (
    <div className="flex items-center justify-center gap-8 py-10">
      ...
      <div className="flex flex-col items-center">
        ...
        <span className="text-slate-500 text-[9px] uppercase font-bold tracking-widest text-center">Página</span>
      </div>
      <button
        onClick={() => { setPage(p => Math.min(data?.info.pages || 1, p + 1)); window.scrollTo({ top: 0, behavior: 'smooth' }); }}
  ...
  if (isError) return (
    <div className="flex flex-col items-center justify-center py-40 text-blue-500 italic">
      ...
      <p className="font-black uppercase tracking-widest text-center">Fallo en la sincronización multiversal</p>
    </div>
  );
  ...
  return (
    <div className="text-white py-10 min-h-screen">
      ...
      <header className="text-center mb-16" data-aos="zoom-in">
        ...
        <p className="text-blue-500 text-[10px] font-black tracking-[1.2em] uppercase opacity-70">AnimeHero Data Engine</p>
      </header>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
        {data?.results.map((char: Character, i: number) => (
          <div key={char.id} data-aos="fade-up" data-aos-delay={(i % 4) * 50} onClick={() => setSelectedCharacter(char)} className="cursor-pointer">
            <Card 
              character={char} 
              onRate={handleRate} 
              rating={ratingsData?.find((r: any) => r.characterId === char.id)?.rating || 0} 
            />
          </div>
        ))}
      </div>
      ...
      {/* MODAL DETALLES */}
      {selectedCharacter && (
        <Modal title={selectedCharacter.name} onClose={() => setSelectedCharacter(null)}>
          <div className="flex flex-col md:flex-row gap-8 p-4 italic uppercase">
            <img src={selectedCharacter.image} className="w-full md:w-1/2 rounded-[2.5rem] border border-white/10" alt="" />
            <div className="flex-1 flex flex-col justify-center gap-4">
               <div className="bg-white/5 p-6 rounded-3xl border border-white/5">
                  <p className="text-blue-500 text-[9px] font-black tracking-widest mb-1">Status</p>
                  <p className={`text-2xl font-black ${selectedCharacter.status === 'Alive' ? 'text-green-400' : 'text-red-500'}`}>{selectedCharacter.status}</p>
               </div>
               <div className="bg-white/5 p-6 rounded-3xl border border-white/5">
                  <p className="text-blue-500 text-[9px] font-black tracking-widest mb-1">Especie</p>
                  <p className="text-2xl font-black text-white">{selectedCharacter.species}</p>
               </div>
            </div>
          </div>
        </Modal>
