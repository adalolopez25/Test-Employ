...
export const Card = ({
  ...
  return (
    <div 
      ...
      className="group relative w-full max-w-[280px] h-[420px] rounded-2xl overflow-hidden shadow-xl cursor-pointer bg-slate-950 transition-all duration-500 ease-[cubic-bezier(0.17,0.67,0.3,1)] hover:-translate-y-4 hover:shadow-2xl hover:shadow-black/60"
    >
      {/* Imagen con Zoom */}
      <img 
        src={character.image} 
      ...
      <div className="absolute inset-0 flex flex-col justify-end p-6 z-20 bg-gradient-to-t from-slate-900 via-slate-900/90 to-transparent translate-y-full transition-transform duration-500 ease-in-out group-hover:translate-y-0">
        <div className="mb-4">
          <h4 className="text-white text-lg font-bold mb-3">{character.name}</h4>
          
          <div className="space-y-1.5 text-xs text-slate-300">
            <p><span className="text-slate-500 uppercase tracking-tighter mr-1">Status:</span> <b className="text-slate-100">{character.status}</b></p>
            <p><span className="text-slate-500 uppercase tracking-tighter mr-1">Species:</span> <b className="text-slate-100">{character.species}</b></p>
            <p><span className="text-slate-500 uppercase tracking-tighter mr-1">Origin:</span> <b className="text-slate-100">{character.origin.name}</b></p>
            <p><span className="text-slate-500 uppercase tracking-tighter mr-1">Location:</span> <b className="text-slate-100">{character.location.name}</b></p>
          </div>
        </div>
