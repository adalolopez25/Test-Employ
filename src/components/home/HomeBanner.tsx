...
import { fetcher } from "@/lib/api-client"; // 1. Importamos tu cliente pro
import Image from "next/image"; // 2. Optimización de imágenes

// 3. Usamos el fetcher centralizado
const fetchFeatured = async () => {
  // Pasamos los IDs directamente. El fetcher ya sabe manejar URLs externas.
...
export function BannerCarousel() {
  ...
  const { data: characters, isLoading } = useQuery({
    ...
    staleTime: 1000 * 60 * 60, // Cache de 1 hora para el banner
  });
  ...
  return (
    ...
    >
        ...
        return (
          <div
            ...
            className={cn(
              ...
              isCurrent ? "opacity-100 translate-x-0 z-20 visible" : "opacity-0 translate-x-12 z-0 invisible"
            )}
          >
            {/* LADO IZQUIERDO: TEXTO */}
            <div className="relative z-50 w-full md:w-[50%] h-full flex flex-col justify-center pl-16 md:pr-10">
              <div className={cn(
                ...
                <Link href={`/info/${char.id}`} className="bg-white text-black px-12 py-5 rounded-full font-black italic uppercase tracking-widest hover:bg-blue-600 hover:text-white transition-all shadow-xl active:scale-95 inline-flex items-center gap-3 w-fit">
                  <Play size={18} fill="currentColor" /> Ver Perfil
                </Link>
              </div>
            </div>

            {/* LADO DERECHO: IMAGEN OPTIMIZADA */}
            <div className={cn(
              "absolute top-0 right-0 w-full md:w-[70%] h-full z-10 flex items-center justify-end overflow-hidden transition-all duration-1500",
              ...
              <div className="relative w-full h-full flex items-center justify-center pr-10 md:pr-24">
                ...
                {/* Usamos Image de Next.js para carga prioritaria */}
                <div className="relative w-[80%] h-[80%]">
                  <Image 
                    src={char.image} 
                    alt={char.name}
                    fill
                    priority={index === 0} // Carga la primera imagen al instante
                    className="object-contain relative z- drop-shadow-[0_0_80px_rgba(37,99,235,0.4)]"
                    style={{ 
      ...
      {/* NAVEGACIÓN */}
      <div className="absolute bottom-12 left-0 w-full px-16 z- flex items-center justify-between">
        <div className="flex gap-2 bg-black/20 backdrop-blur-md p-3 rounded-full border border-white/5">
        ...
        <div className="flex gap-4">
          <button onClick={prev} className="p-4 rounded-full bg-white/5 backdrop-blur-xl border border-white/10 hover:bg-blue-600 text-white transition-all group active:scale-90">
            <ChevronLeft size={20} className="group-hover:-translate-x-1 transition-transform" />
          </button>
          <button onClick={next} className="p-4 rounded-full bg-white/5 backdrop-blur-xl border border-white/10 hover:bg-blue-600 text-white transition-all group active:scale-90">
